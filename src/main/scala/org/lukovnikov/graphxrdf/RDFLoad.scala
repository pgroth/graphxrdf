...
object RDFLoad {
	...
	def rwr(graph:Graph[String,String], numiter:Int = 2) = {
		var outdegrees = graph.outDegrees.map(
				outdeg => 
					(outdeg._1, (outdeg._2, Map[Long,Double](), Map[Long, Double](outdeg._1 -> 1.0)))
				)
		var g2 = Graph[(Int,Map[Long, Double],Map[Long, Double]), String](outdegrees, graph.edges)
		for (vertex <- outdegrees.collect)
			Console.println(vertex)
		//g2 = g2.mapEdges(edge => null)
		var iter = numiter
		while (iter > 0) {
			val agg = g2.aggregateMessages[(Int,Map[Long, Double],Map[Long,Double])](
				triplet => {
					...
					if (triplet.dstAttr != null)
						for (entry <- triplet.dstAttr._3) {
							ret += (entry._1 -> entry._2/sourcedeg)
						}
					triplet.sendToSrc((sourcedeg, triplet.srcAttr._2, ret))
				},
				(a,b) => {
					// merge inboxes
					val degree = a._1
					...
					(degree, Map() ++ state, Map() ++ inbox)
				}
			)
			g2 = Graph(agg, g2.edges)
			iter -= 1
		}
		val origvertices = graph.vertices.collect
		val aggrvertices = g2.vertices.collect
